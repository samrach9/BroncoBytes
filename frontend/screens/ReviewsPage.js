...
export default function ReviewsPage({ route }) {
  ...
  const foundUser = allUsers.find((item) => item.userId == food.foodId);
  ...
  return (
    <View style={styles.container}>
      <ScrollView style={styles.contentContainer}>
        ...
        <View style={styles.content}>
          {food.foodId == user.userId ? (
            ...
            <BigRectangleButton
              text={"Friends"}
              onClick={() => navigation.navigate("Friends")}
            />
          ) : (
            <FoodCard food={food} />
          )}
          {reviews.map((item) => {
            return (
              <ReviewCard
                key={item.reviewId}
                review={item}
                canRemove={user.admin || user.userId == item.userId}
              />
            );
          })}
        </View>
      ...
      <Footer
        ...
        rightButtonPress={() =>
          navigation.navigate("Leave Review", { foodId: food.foodId })
        }
      />
    </View>
  );
